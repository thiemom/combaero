...
9. **C.2** — C++ multi-layer `wall_coupling_and_jacobian` overload + pybind
10. **C.4** — Solver wiring for multi-layer walls
11. **C.5** — Post-solve wall temperature profile
12. **C.3** — Explicit `has_convective_surface` property
13. **C.6** — GUI schema + graph builder for wall layers
