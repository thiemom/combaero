...
from combaero.network import ConstantLoss, LinearThetaLoss
...
class TestConstantLoss:
    def test_returns_zeta0_and_zero_derivative(self) -> None:
        loss = ConstantLoss(zeta0=0.03)
        zeta, dzeta_dtheta = loss(_Ctx(theta=0.0))
        assert zeta == pytest.approx(0.03)
        assert dzeta_dtheta == pytest.approx(0.0)

    def test_independent_of_theta(self) -> None:
        loss = ConstantLoss(zeta0=0.05)
        for theta in [-1.0, 0.0, 0.5, 2.0, 10.0]:
            zeta, dz = loss(_Ctx(theta=theta))
            assert zeta == pytest.approx(0.05)
            assert dz == pytest.approx(0.0)

    def test_default_zeta0(self) -> None:
        loss = ConstantLoss()
        zeta, _ = loss(_Ctx())
        assert zeta == pytest.approx(0.03)

    def test_zero_loss(self) -> None:
        loss = ConstantLoss(zeta0=0.0)
        zeta, dz = loss(_Ctx(theta=1.5))
...
class TestLinearThetaLoss:
    def test_at_zero_theta(self) -> None:
        loss = LinearThetaLoss(k=0.5, zeta0=0.02)
        zeta, dz = loss(_Ctx(theta=0.0))
        assert zeta == pytest.approx(0.02)
        assert dz == pytest.approx(0.5)

    def test_linear_slope(self) -> None:
        loss = LinearThetaLoss(k=0.4, zeta0=0.01)
        theta = 2.0
        zeta, dz = loss(_Ctx(theta=theta))
        assert zeta == pytest.approx(0.4 * theta + 0.01)
        assert dz == pytest.approx(0.4)

    def test_derivative_constant_wrt_theta(self) -> None:
        loss = LinearThetaLoss(k=0.6, zeta0=0.03)
        for theta in [0.0, 0.5, 1.0, 3.0]:
            _, dz = loss(_Ctx(theta=theta))
            assert dz == pytest.approx(0.6)

    def test_default_values(self) -> None:
        loss = LinearThetaLoss()
        zeta, dz = loss(_Ctx(theta=1.0))
        assert zeta == pytest.approx(0.5 * 1.0 + 0.02)
        assert dz == pytest.approx(0.5)

    def test_fd_consistency(self) -> None:
        """Analytical derivative matches finite difference."""
        loss = LinearThetaLoss(k=0.35, zeta0=0.015)
        theta = 1.5
        eps = 1e-6
        zeta0, dz = loss(_Ctx(theta=theta))
        zeta_p, _ = loss(_Ctx(theta=theta + eps))
        fd = (zeta_p - zeta0) / eps
        assert dz == pytest.approx(fd, rel=1e-5)


# ---------------------------------------------------------------------------
# Integration test: CombustorNode + LinearThetaLoss reduces P_total
# ---------------------------------------------------------------------------
...
class TestCombustorNodeWithPressureLoss:
    ...
    def test_constant_loss_reduces_P_total(self) -> None:
        ...
        fuel, air = self._make_streams()
        node = CombustorNode("comb", method="complete", pressure_loss=ConstantLoss(zeta0=0.05))
        _, _, mix_res = node.compute_derived_state([fuel, air])
    ...
    def test_linear_theta_loss_reduces_P_total(self) -> None:
        ...
        node_loss = CombustorNode(
            "loss", method="complete", pressure_loss=LinearThetaLoss(k=0.5, zeta0=0.02)
        )
