"""Tests for composite pressure_drop_channel function.
...
class TestPressureDropChannelComposite:
    """Test composite pressure_drop_channel function (Phase 4)."""
    ...
    def test_basic_functionality(self):
        ...
        dP, Re, f = cb.pressure_drop_channel(T, P, Y_air, v, D, L)
        ...
        # Sanity checks for air at 10 m/s in 0.1m channel
    ...
    def test_matches_manual_calculation(self):
        ...
        # Composite function
        dP_comp, Re_comp, f_comp = cb.pressure_drop_channel(
            T, P, Y_air, v, D, L, roughness, "haaland"
        )
        ...
        dP_manual = cb.channel_dP(v, L, D, f_manual, rho)
    ...
    def test_all_correlations(self):
        ...
        for corr in correlations:
            dP, Re, f = cb.pressure_drop_channel(T, P, Y_air, v, D, L, roughness, corr)
        ...
        # Rough-channel correlations should agree closely
        ...
        # Petukhov (smooth-channel) should give lower f than rough-channel correlations
        assert results["petukhov"][2] < f_rough_mean

    def test_smooth_vs_rough_channel(self):
        """Test that rough channels have higher friction factor."""
        T = 300.0  # K
        P = 101325.0  # Pa
        Y_air = cb.species.dry_air()
        v = 12.0  # m/s
        ...
        # Smooth channel
        dP_smooth, Re_smooth, f_smooth = cb.pressure_drop_channel(
            T, P, Y_air, v, D, L, roughness=0.0
        )

        # Rough channel (commercial steel)
        dP_rough, Re_rough, f_rough = cb.pressure_drop_channel(
        ...
        # Rough channel should have higher friction factor
        assert f_rough > f_smooth

        # Rough channel should have higher pressure drop
    ...
    def test_velocity_dependence(self):
        ...
        dP1, Re1, f1 = cb.pressure_drop_channel(T, P, Y_air, v1, D, L)
        dP2, Re2, f2 = cb.pressure_drop_channel(T, P, Y_air, v2, D, L)
    ...
    def test_length_dependence(self):
        ...
        dP1, Re1, f1 = cb.pressure_drop_channel(T, P, Y_air, v, D, L1)
        dP2, Re2, f2 = cb.pressure_drop_channel(T, P, Y_air, v, D, L2)
    ...
    def test_temperature_effect(self):
        ...
        dP_cold, Re_cold, f_cold = cb.pressure_drop_channel(T_cold, P, Y_air, v, D, L)
        dP_hot, Re_hot, f_hot = cb.pressure_drop_channel(T_hot, P, Y_air, v, D, L)
    ...
    def test_pressure_effect(self):
        ...
        dP_low, Re_low, f_low = cb.pressure_drop_channel(T, P_low, Y_air, v, D, L)
        dP_high, Re_high, f_high = cb.pressure_drop_channel(T, P_high, Y_air, v, D, L)
    ...
    def test_default_parameters(self):
        """Test default parameters (smooth channel, haaland correlation)."""
        T = 300.0  # K
        P = 101325.0  # Pa
        Y_air = cb.species.dry_air()
        v = 10.0  # m/s
        ...
        # Call with defaults
        dP_default, Re_default, f_default = cb.pressure_drop_channel(T, P, Y_air, v, D, L)

        # Call with explicit defaults
        dP_explicit, Re_explicit, f_explicit = cb.pressure_drop_channel(
    ...
    def test_invalid_correlation(self):
        ...
        with pytest.raises(ValueError, match="unknown friction correlation"):
            cb.pressure_drop_channel(T, P, Y_air, v, D, L, correlation="invalid")
