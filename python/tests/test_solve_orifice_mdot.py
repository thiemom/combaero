...
class TestSolveOrificeMdot:
    ...
    def test_compressible_flow(self):
        ...
        mdot_comp = cb.solve_orifice_mdot(
            geom, dP, rho, mu, P_upstream, kappa
        )
        
        # Compressible flow should give slightly lower mdot due to expansion
        mdot_incomp = cb.solve_orifice_mdot(
            geom, dP, rho, mu, P_upstream, 0.0
        )
    ...
    def test_different_correlations(self):
        ...
        for corr in correlations:
            mdot = cb.solve_orifice_mdot(
                geom, dP, rho, mu, correlation=corr
            )
            results[corr] = mdot
            assert mdot > 0
    ...
    def test_custom_tolerance(self):
        ...
        # Tight tolerance
        mdot_tight = cb.solve_orifice_mdot(
            geom, dP, rho, mu, tol=1e-8
        )
        
        # Loose tolerance
        mdot_loose = cb.solve_orifice_mdot(
            geom, dP, rho, mu, tol=1e-4
        )
