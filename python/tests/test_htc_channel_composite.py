"""Tests for composite htc_channel function.
...
class TestHtcChannelComposite:
    """Test composite htc_channel function with correlation selection."""

    def test_htc_channel_gnielinski_default(self):
        """Test htc_channel with default Gnielinski correlation."""
        # Air at 300K, 1 atm
        T = 300.0  # K
        P = 101325.0  # Pa
        ...
        h, Nu, Re = cb.htc_channel(T, P, X, v, D)
    ...
    def test_htc_channel_decomposition_gnielinski(self):
        ...
        # Composite call
        h_comp, Nu_comp, Re_comp = cb.htc_channel(T, P, X, v, D, correlation="gnielinski")
    ...
    def test_htc_channel_decomposition_dittus_boelter(self):
        ...
        # Composite call
        h_comp, Nu_comp, Re_comp = cb.htc_channel(
    ...
    def test_htc_channel_decomposition_sieder_tate(self):
    ...
    def test_htc_channel_decomposition_petukhov(self):
        ...
        # Composite call
        h_comp, Nu_comp, Re_comp = cb.htc_channel(T, P, X, v, D, correlation="petukhov")
    ...
    def test_htc_channel_heating_vs_cooling(self):
        ...
        h_heat, Nu_heat, Re_heat = cb.htc_channel(
            T, P, X, v, D, correlation="dittus_boelter", heating=True
        )
        h_cool, Nu_cool, Re_cool = cb.htc_channel(
    ...
    def test_htc_channel_with_roughness(self):
        """Test htc_channel with channel roughness."""
        T = 300.0  # K
        P = 101325.0  # Pa
        X = cb.species.dry_air()
        v = 10.0  # m/s
        ...
        h_rough, Nu_rough, Re_rough = cb.htc_channel(
            T, P, X, v, D, correlation="gnielinski", roughness=roughness
        )

        h_smooth, Nu_smooth, Re_smooth = cb.htc_channel(
        ...
        # Rough channel should have higher friction, thus higher Nu and h
        assert Nu_rough > Nu_smooth
        assert h_rough > h_smooth

    def test_htc_channel_laminar_flow(self):
        """Test htc_channel handles laminar flow (Re < 2300)."""
        T = 300.0  # K
        P = 101325.0  # Pa
        X = cb.species.dry_air()
        ...
        h, Nu, Re = cb.htc_channel(T, P, X, v, D, correlation="gnielinski")
    ...
    def test_htc_channel_temperature_effect(self):
        ...
        h_low, Nu_low, Re_low = cb.htc_channel(T_low, P, X, v, D)
        h_high, Nu_high, Re_high = cb.htc_channel(T_high, P, X, v, D)
    ...
    def test_htc_channel_velocity_effect(self):
        ...
        h_low, Nu_low, Re_low = cb.htc_channel(T, P, X, v_low, D)
        h_high, Nu_high, Re_high = cb.htc_channel(T, P, X, v_high, D)
    ...
    def test_htc_channel_diameter_effect(self):
        ...
        h_small, Nu_small, Re_small = cb.htc_channel(T, P, X, v, D_small)
        h_large, Nu_large, Re_large = cb.htc_channel(T, P, X, v, D_large)
    ...
    def test_htc_channel_invalid_correlation(self):
...
class TestHtcChannelEdgeCases:
    """Test edge cases and error handling."""

    def test_htc_channel_negative_velocity(self):
        ...
        with pytest.raises(ValueError, match="velocity must be positive"):
            cb.htc_channel(T, P, X, v, D)

    def test_htc_channel_zero_diameter(self):
    ...
    def test_htc_channel_negative_temperature(self):
