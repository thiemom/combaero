...
def test_plenum_node_creation():
    ...
    # Should have P and Pt unknowns (Derived-State architecture)
    unknowns = plenum.unknowns()
    assert "test_plenum.P" in unknowns
    assert "test_plenum.Pt" in unknowns
...
def test_momentum_chamber_node_creation():
    ...
    assert "test_chamber.Pt" in unknowns
...
def test_combustor_node_creation():
    ...
    assert "test_combustor.Pt" in unknowns
...
def test_plenum_with_multiple_upstream():
    ...
    # Now should only have P and Pt unknowns (Pure Pressure-Flow)
    unknowns = plenum.unknowns()
    assert "mixing_plenum.P" in unknowns
    assert "mixing_plenum.Pt" in unknowns
...
def test_chamber_pressure_relationship():
    ...
    # Should have one residual: Pt - P = 0 (Legacy path)
    ...
    assert residuals_mismatch[0] == 1000.0  # Pt - P = 101000 - 100000 = 1000
...
def test_chamber_integration_with_network():
    ...
    # Create nodes
    inlet = PressureBoundary("inlet", Pt=500000.0, Tt=700.0)
    plenum = PlenumNode("plenum")
    chamber = MomentumChamberNode("chamber", area=0.1)
    combustor = CombustorNode("combustor")
    outlet = PressureBoundary("outlet", Pt=101325.0, Tt=400.0)
