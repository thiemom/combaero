...
X = np.zeros(cb.species.num_species, dtype=float)
X[cb.species_index_from_name("CH4")] = 0.095
X[cb.species_index_from_name("O2")] = 0.19
X[cb.species_index_from_name("N2")] = 0.715

# Adiabatic complete combustion
burned = cb.complete_combustion(T=300.0, X=X)
print(f"Adiabatic flame T: {burned.T:.0f} K")
print(f"Flame density: {burned.rho:.4f} kg/m³")

# WGS equilibrium (isothermal or adiabatic)
eq = cb.wgs_equilibrium_adiabatic(T=1500.0, X=burned.X)
...
X = np.zeros(cb.species.num_species, dtype=float)
X[cb.species_index_from_name("CH4")] = 0.02    # Unburned methane
X[cb.species_index_from_name("C2H6")] = 0.003  # Unburned ethane
X[cb.species_index_from_name("H2O")] = 0.20
X[cb.species_index_from_name("CO2")] = 0.08
X[cb.species_index_from_name("N2")] = 0.70

# General reforming equilibrium (handles all hydrocarbons)
# CnHm + n*H2O <-> n*CO + (n + m/2)*H2
# CO + H2O <-> CO2 + H2 (WGS)
eq = cb.reforming_equilibrium_adiabatic(T=2200.0, X=X)
print(f"Equilibrium T: {eq.T:.0f} K")
print(f"CO: {eq.X[cb.species_index_from_name('CO')]*100:.2f}%")
print(f"H2: {eq.X[cb.species_index_from_name('H2')]*100:.2f}%")

# Isothermal version (fixed temperature)
eq_iso = cb.reforming_equilibrium(T=2000.0, X=X)

# SMR+WGS (CH4 only, for backward compatibility)
eq_smr = cb.smr_wgs_equilibrium_adiabatic(T=2200.0, X=X)
```
...
result = cb.combustion_equilibrium(T=300.0, X=X_unburned)
...
P_ratio = cb.critical_pressure_ratio(T0, P0, X)
...
# Basic transport
print(f"Viscosity: {cb.viscosity(T, P, X):.2e} Pa·s")
print(f"Thermal conductivity: {cb.thermal_conductivity(T, P, X):.4f} W/(m·K)")
print(f"Prandtl: {cb.prandtl(T, P, X):.3f}")
...
print(cb.common_name("N2"))   # "Nitrogen"

# Common name to formula
print(cb.formula("Methane"))  # "CH4"
print(cb.formula("Water"))    # "H2O"

# Get full mappings
formula_map = cb.formula_to_name()  # dict: formula -> name
name_map = cb.name_to_formula()     # dict: name -> formula
```
...
# Bernoulli equation
P2 = cb.bernoulli_P2(P1=200000, v1=2.0, v2=5.0, rho=rho)
v2 = cb.bernoulli_v2(P1=200000, P2=150000, v1=2.0, rho=rho)
...
sol_rib = cb.channel_ribbed(T, P, X, u, D, L,
...
sol_dim = cb.channel_dimpled(T, P, X, u, D, L, d_Dh=0.2, h_d=0.2, S_d=2.0)

# Pin-fin array (Metzger et al. 1982)
sol_pin = cb.channel_pin_fin(T, P, X, u,
...
fmul = cb.rib_friction_multiplier(e_D=0.05, pitch_to_height=10.0)

# High-Re direct empirical fit (Singh & Ekkad 2017, Re = 30k-400k)
# Nu/Nu0 = 54.9 * Re^(-0.1755) * (e/D)^0.38 * (P/e)^(-0.11) * F(alpha)
enh_hre = cb.rib_enhancement_factor_high_re(e_D=0.0625, pitch_to_height=10.0,
                                             alpha_deg=60.0, Re=200_000.0)
fmul_hre = cb.rib_friction_multiplier_high_re(e_D=0.0625, pitch_to_height=10.0)
...
eta = cb.thermal_performance_factor(Nu_ratio=enh_hre, f_ratio=fmul_hre)

# Impingement Nusselt (Martin 1977 / Florschuetz 1981)
Nu = cb.impingement_nusselt(Re_jet=20000, Pr=0.7, z_D=6.0)
Nu_arr = cb.impingement_nusselt(Re_jet=20000, Pr=0.7, z_D=6.0, x_D=8.0, y_D=8.0)

# Film cooling effectiveness (Baldauf et al. 2002)
eta = cb.film_cooling_effectiveness(x_D=10.0, M=1.0, DR=1.5, alpha_deg=30.0)
eta_avg = cb.film_cooling_effectiveness_avg(x_D=10.0, M=1.0, DR=1.5,
                                             alpha_deg=30.0, s_D=3.0)

# Effusion effectiveness
eta_eff = cb.effusion_effectiveness(x_D=5.0, M=0.5, DR=1.5,
...
k = cb.k_haynes230(T=1100)      # W/(m*K), valid 300-1400 K
...
k_fresh = cb.k_tbc_ysz(T=1200, hours=0)       # As-sprayed
k_aged  = cb.k_tbc_ysz(T=1200, hours=1000)    # After 1000 h
k_ebpvd = cb.k_tbc_ysz(T=1200, is_ebpvd=True) # EB-PVD process

# List all available materials
print(cb.list_materials())
```
