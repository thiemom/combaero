...
try:  # Python 3.8+
    ...
    from importlib_metadata import \
        version as _pkg_version  # type: ignore[import-not-found]
...
try:
    ...
    from ._core import (AcousticMode, Annulus,  # type: ignore[attr-defined]
                        BoundaryCondition, Cd_orifice, Cd_rounded_entry,
                        Cd_sharp_thin_plate, Cd_thick_plate,
                        CompressibleFlowSolution, FannoSolution, FannoStation,
                        NozzleSolution, NozzleStation, OrificeGeometry,
                        OrificeState, State, Stream, ThrustResult, Tube,
                        UnitInfo, adiabatic_T_wgs, all_units,
                        annulus_axial_modes, annulus_azimuthal_modes,
                        annulus_modes, axial_mode_downstream, axial_mode_split,
                        axial_mode_upstream, bandwidth, bernoulli_P2,
                        bernoulli_v2, bilger_stoich_mixture_fraction_mass,
                        bilger_Z_from_equivalence_ratio_mass,
                        bulk_T_from_edge_T_and_q, calc_T_from_cp,
                        calc_T_from_h, calc_T_from_s, capacity_ratio,
                        closest_mode, combustion_equilibrium, common_name,
                        complete_combustion, complete_combustion_isothermal,
                        complete_combustion_to_CO2_H2O,
                        complete_combustion_to_CO2_H2O_with_fraction,
                        convert_to_dry_fractions, cp, critical_pressure_ratio,
                        cv, damping_ratio, density, dewpoint, dT_edge_dq,
                        dT_edge_dT_bulk, dT_edge_dT_cold, dT_edge_dT_hot,
                        dynamic_pressure, effective_viscothermal_layer,
                        effectiveness_counterflow, effectiveness_parallelflow,
                        equivalence_ratio_from_bilger_Z_mass,
                        equivalence_ratio_mass, equivalence_ratio_mole,
                        fanno_max_length, fanno_pipe, formula, formula_to_name,
                        frequency_from_strouhal, friction_colebrook,
                        friction_haaland, friction_petukhov,
                        friction_serghides, get_units, h, half_wave_frequency,
                        has_units, heat_flux, heat_flux_from_T_at_depth,
                        heat_flux_from_T_at_edge, heat_rate,
                        heat_rate_from_effectiveness, heat_transfer_area,
                        heat_transfer_dT, helmholtz_frequency, helmholtz_Q,
                        htc_from_nusselt, humid_air_composition,
                        hydraulic_diameter, hydraulic_diameter_annulus,
                        hydraulic_diameter_rect, input_units,
                        isentropic_expansion_coefficient, kinematic_viscosity,
                        list_functions_with_units, lmtd, lmtd_counterflow,
                        lmtd_parallelflow, mach_from_pressure_ratio,
                        mass_flux_isentropic, mass_to_mole,
                        min_mode_separation, mix, mixture_h, modes_in_range,
                        molar_volume, mole_to_mass, name_to_formula,
                        normalize_fractions, nozzle_cd, nozzle_flow,
                        nozzle_thrust, nozzle_thrust_cd, ntu,
                        nusselt_dittus_boelter, nusselt_gnielinski,
                        nusselt_petukhov, nusselt_sieder_tate, orifice_area,
                        orifice_Cd_from_K, orifice_Cd_from_measurement,
                        orifice_dP, orifice_dP_Cd, orifice_K_from_Cd,
                        orifice_mdot, orifice_mdot_Cd, orifice_Q,
                        orifice_thickness_correction, orifice_velocity,
                        output_units, overall_htc, overall_htc_wall,
                        overall_htc_wall_multilayer,
                        oxygen_required_per_kg_fuel,
                        oxygen_required_per_kg_mixture,
                        oxygen_required_per_mol_fuel,
                        oxygen_required_per_mol_mixture, peclet, pipe_dP,
                        pipe_dP_mdot, pipe_mdot, pipe_velocity, prandtl,
                        quarter_wave_frequency, reforming_equilibrium,
                        reforming_equilibrium_adiabatic,
                        relative_humidity_from_dewpoint, residence_time,
                        residence_time_annulus, residence_time_mdot,
                        residence_time_tube, reynolds, s,
                        set_equivalence_ratio_mass, set_equivalence_ratio_mole,
                        set_fuel_stream_for_CO2, set_fuel_stream_for_CO2_dry,
                        set_fuel_stream_for_O2, set_fuel_stream_for_O2_dry,
                        set_fuel_stream_for_Tad, set_oxidizer_stream_for_CO2,
                        set_oxidizer_stream_for_CO2_dry,
                        set_oxidizer_stream_for_O2,
                        set_oxidizer_stream_for_O2_dry,
                        set_oxidizer_stream_for_Tad, smr_wgs_equilibrium,
                        smr_wgs_equilibrium_adiabatic, solve_A_eff_from_mdot,
                        solve_P0_from_mdot, solve_P_back_from_mdot,
                        space_velocity, specific_gas_constant, speed_of_sound,
                        standard_dry_air_composition, stokes_layer, strouhal,
                        thermal_conductivity, thermal_diffusivity,
                        thermal_layer, thermal_resistance,
                        thermal_resistance_wall, tube_axial_modes, tube_Q,
                        velocity_from_q, viscosity, wall_temperature_profile,
                        wgs_equilibrium, wgs_equilibrium_adiabatic)
