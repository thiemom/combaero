...
constexpr double coeff_prandtl_heating_exp = 0.4;
constexpr double coeff_prandtl_cooling_exp = 0.3;
} // namespace dittus_boelter
// Parameters:
//   Re      : Reynolds number [-]
//   Pr      : Prandtl number [-]
...
inline double heat_rate(double U, double A, double dT) { return U * A * dT; }

// Heat flux q = U * ΔT  [W/m²]
// (heat rate per unit area)
inline double heat_flux(double U, double dT) { return U * dT; }
...
//
// Note: T_wall_hot_surface = T_hot - q/h_hot  (hot wall surface, inside
// boundary layer)
//       T_wall_cold_surface = last element     (cold wall surface, inside
//       boundary layer) To recover cold bulk: T_cold_bulk = T_wall_cold_surface
//       - q/h_cold
//
// Parameters:
...
//          = T_hot * (1 - R_hot_to_edge/R_total) + T_cold *
//          (R_hot_to_edge/R_total)
//
// Therefore:
...
inline double ntu(double U, double A, double C_min) { return U * A / C_min; }
...
//                             (T_aw is between T_static and T_total; using
//                             T_total
...
struct ChannelResult {
  ...
  double
      Re; // Reynolds number [-]  (hydraulic D or pin D depending on geometry)
...
//   x_D      : streamwise jet spacing / d_jet [-]  (0 = single jet; valid
//   array: 4-16) y_D      : spanwise jet spacing / d_jet [-]    (0 = single
