...
int main() {
  ...
  // Fuel stream: pure methane at 300 K (mdot will be set by
  // set_fuel_stream_for_phi)
  std::vector<double> X_fuel(n_species, 0.0);
  X_fuel[idx_CH4] = 1.0;
  Stream fuel;
  ...
  Stream air;
  air.state.set_TPX(300.0, 101325.0,
                    humid_air_composition(300.0, 101325.0, 0.60));
  ...
  std::cout << std::setw(6) << "phi" << std::setw(10) << "mdot_f"
            << std::setw(10) << "T_mix" << std::setw(10) << "T_eq"
            << std::setw(12) << "mu_eq" << std::setw(12) << "Pr_eq"
            << "\n";
  std::cout << std::setw(6) << "[-]" << std::setw(10) << "[kg/s]"
            << std::setw(10) << "[K]" << std::setw(10) << "[K]" << std::setw(12)
            << "[uPa.s]" << std::setw(12) << "[-]"
            << "\n";
  ...
  for (double phi = 0.5; phi <= 1.21; phi += 0.1) {
    ...
    // Output results
    std::cout << std::setw(6) << phi << std::setw(10) << fuel_phi.mdot
              << std::setw(10) << mixed.state.T << std::setw(10) << eq.state.T
  ...
  std::cout << "Product Composition (Reforming+WGS equilibrium, mole fractions "
               "> 0.001):\n";
