...
double friction_haaland(double Re, double e_D) {
  ...
  double term =
      std::pow(e_D / haaland::coeff_roughness, haaland::coeff_exponent) +
      haaland::coeff_reynolds / Re;
  double inv_sqrt_f = haaland::coeff_outer * std::log10(term);
...
double friction_colebrook(double Re, double e_D, double tol, int max_iter) {
  ...
  // Initial guess from Serghides (<0.3% error vs Colebrook, better than Haaland
  // ~2-3%)
