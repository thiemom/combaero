...
double saturation_vapor_pressure(double T) {
  ...
  if (t <= 0.0) {
    ...
    P_ws = std::exp(WATER_A - WATER_B / (t + WATER_D1)) /
           std::pow(t + WATER_D2, WATER_C);
  }
...
double dewpoint(double T, [[maybe_unused]] double P, double RH) {
  ...
  for (std::size_t i = 0; i < max_iterations; i++) {
    ...
    // Calculate approximate derivative of saturation vapor pressure with
    // respect to T Use central difference approximation
    const double delta =
        0.01; // Small temperature difference for derivative calculation
...
double relative_humidity_from_dewpoint(double T, double Tdp, double P) {
  ...
  if (Tdp > T) {
    throw std::invalid_argument(
        "relative_humidity_from_dewpoint: Tdp cannot exceed T");
  }
...
static double d_saturation_vapor_pressure_dT(double T) {
  ...
  return (saturation_vapor_pressure(T + delta) -
          saturation_vapor_pressure(T - delta)) /
         (2.0 * delta);
}
...
double wet_bulb_temperature(double T, double P, double RH) {
  ...
  throw std::runtime_error(
      "wet_bulb_temperature: Newton-Raphson did not converge");
}
...
double humid_air_enthalpy(double T, double P, double RH) {
  ...
  double h_da =
      1005.0 * (T - 273.15); // cp_air * (T - T_ref), cp_air = 1005 J/(kg·K)
...
// air] Uses h(T, X) from the NASA-9 polynomial database, converted to
// per-kg-dry-air basis.
...
// -------------------------------------------------------------------------
// HumidAir State Object Implementation
// -------------------------------------------------------------------------

HumidAir &HumidAir::set_TP_RH(double T, double P, double RH_new) {
  if (RH_new < 0.0 || RH_new > 1.0) {
    throw std::invalid_argument("HumidAir: RH must be between 0 and 1");
  }
  rh = RH_new;
  std::vector<double> X = humid_air_composition(T, P, rh);
  state.set_TPX(T, P, X);
  return *this;
}

HumidAir &HumidAir::set_TP_dewpoint(double T, double P, double Tdp) {
  double RH_new = relative_humidity_from_dewpoint(T, Tdp, P);
  return set_TP_RH(T, P, RH_new);
}

HumidAir &HumidAir::set_TP_omega(double T, double P, double w) {
  if (w < 0.0) {
    throw std::invalid_argument(
        "HumidAir: humidity ratio must be non-negative");
  }
  // w = 0.621945 * P_w / (P - P_w)
  // P_w = w * P / (0.621945 + w)
  double P_w = w * P / (0.621945 + w);
  double P_ws = saturation_vapor_pressure(T);
  double RH_new = P_w / P_ws;
  return set_TP_RH(T, P, RH_new);
}
...
std::tuple<double, double, double> HumidAir::TP_RH() const {
  return std::make_tuple(state.T, state.P, rh);
}
