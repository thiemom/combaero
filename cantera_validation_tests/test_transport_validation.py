...
class TestTransportProperties:
    ...
    def test_viscosity_temperature_variation(self, combaero, cantera, gri30_gas, species_mapping, tolerance_config):
        ...
        for T in [300.0, 500.0, 1000.0, 1500.0, 2000.0]:
            ...
            rel_diff = abs(mu_cb - mu_ct) / mu_ct
            assert rel_diff < tolerance_config["transport"], \
                f"T={T} K: CombAero={mu_cb:.2e} Pa·s, Cantera={mu_ct:.2e} Pa·s, diff={rel_diff*100:.1f}%"

    def test_methane_viscosity(self, combaero, cantera, gri30_gas, species_mapping, tolerance_config):
    ...
    def test_air_thermal_conductivity(self, combaero, cantera, gri30_gas, species_mapping, tolerance_config):
        ...
        assert rel_diff < tolerance_config["transport"], \
            f"Thermal conductivity [W/(m·K)]: CombAero={k_cb:.4f}, Cantera={k_ct:.4f}, diff={rel_diff*100:.1f}%"

    def test_thermal_conductivity_temperature_variation(self, combaero, cantera, gri30_gas, species_mapping, tolerance_config):
        ...
        for T in [300.0, 500.0, 1000.0, 1500.0, 2000.0]:
            ...
            rel_diff = abs(k_cb - k_ct) / k_ct
            assert rel_diff < tolerance_config["transport"], \
                f"T={T} K: CombAero={k_cb:.4f} W/(m·K), Cantera={k_ct:.4f} W/(m·K), diff={rel_diff*100:.1f}%"

    def test_methane_thermal_conductivity(self, combaero, cantera, gri30_gas, species_mapping, tolerance_config):
    ...
    def test_prandtl_number(self, combaero, cantera, gri30_gas, species_mapping, tolerance_config):
        ...
        assert rel_diff < tolerance_config["transport"], \
            f"Prandtl number [dimensionless]: CombAero={Pr_cb:.4f}, Cantera={Pr_ct:.4f}, diff={rel_diff*100:.1f}%"

    def test_mixture_transport_properties(self, combaero, cantera, gri30_gas, species_mapping, tolerance_config):
    ...
    def test_high_temperature_transport(self, combaero, cantera, gri30_gas, species_mapping, tolerance_config):
        ...
        assert mu_rel_diff < tolerance_config["transport"], \
            f"High-T viscosity mismatch at {burned.T:.0f} K: CombAero={mu_cb:.2e} Pa·s, Cantera={mu_ct:.2e} Pa·s"
        assert k_rel_diff < tolerance_config["transport"], \
            f"High-T thermal conductivity mismatch at {burned.T:.0f} K: CombAero={k_cb:.4f} W/(m·K), Cantera={k_ct:.4f} W/(m·K)"
...
class TestThermodynamicProperties:
    ...
    def test_cp_temperature_variation(self, combaero, cantera, gri30_gas, species_mapping, tolerance_config):
