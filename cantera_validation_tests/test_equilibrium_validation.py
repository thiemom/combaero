...
class TestWgsEquilibrium:
    ...
    def test_wgs_isothermal_high_temp(self, combaero, cantera, wgs_gas, species_mapping, tolerance_config):
    ...
    def test_wgs_isothermal_low_temp(self, combaero, cantera, wgs_gas, species_mapping, tolerance_config):
        ...
        for species in ["CO", "H2O", "CO2", "H2"]:
            ...
            if X_cb > 0.01 or X_ct > 0.01:
                assert diff < 0.005, \
                    f"{species} [mol/mol]: CombAero={X_cb:.6f}, Cantera={X_ct:.6f}"
    ...
    def test_wgs_adiabatic(self, combaero, cantera, wgs_gas, species_mapping, tolerance_config):
        ...
        print(f"  T_final: CombAero={result_cb.T:.1f} K, Cantera={wgs_gas.T:.1f} K, diff={T_diff:.1f} K")
    ...
    def test_wgs_equilibrium_constant(self, combaero, cantera, wgs_gas, species_mapping):
        ...
        for T in [800.0, 1000.0, 1200.0, 1500.0]:
            ...
            Kp_cb = (X_CO2_cb * X_H2_cb) / (X_CO_cb * X_H2O_cb) if (X_CO_cb * X_H2O_cb) > 1e-10 else 0.0
            ...
            Kp_ct = (X_CO2_ct * X_H2_ct) / (X_CO_ct * X_H2O_ct) if (X_CO_ct * X_H2O_ct) > 1e-10 else 0.0
            
            rel_diff = abs(Kp_cb - Kp_ct) / Kp_ct if Kp_ct > 1e-10 else 0.0
            
            print(f"  T = {T} K: CombAero Kp={Kp_cb:.4f}, Cantera Kp={Kp_ct:.4f}, diff={rel_diff*100:.2f}%")
