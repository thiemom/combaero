...
- **Versioning is fully git-tag-driven (`setuptools_scm`) — there is no version string to hand-edit.** Both `pyproject.toml` (combaero) and `gui/pyproject.toml` (combaero-gui) declare `dynamic = ["version"]`; the version comes from the nearest `v*` git tag. Cutting a release is: cut `CHANGELOG.md`'s `[Unreleased]` into a dated `[x.y.z]` block (update the compare-links at the bottom too) → PR → merge to `main` → `git tag -s vx.y.z <commit> -m "vx.y.z" && git push origin vx.y.z`. Use `-s`, not `-a`: an annotated tag carries its own signature, and GitHub reports the TAG's status rather than the signed commit underneath it, so an unsigned annotated tag shows as "Unverified" even on a signed commit (this is what happened to v0.4.0 through v0.5.0). `-s` fails loudly where `-a` would silently produce an unsigned tag.
