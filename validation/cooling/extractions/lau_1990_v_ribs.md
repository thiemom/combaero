...
on two independent channels throughout and three where it mattered; no cell
rests on a single read. The V-shaped cases are transcribed and NOT cleared for
implementation -- see "For the reviewer".**
...
| 7 | Table 2 | seven cases, `R`/`G`/`G_bar,Lau` each as `a(e+)^b` -- reproduced below | **two channels on all 42 numbers; three on four of the six columns** (`R` a, `R` b, `G` b) plus the 7/-V-135 row, which carries the remaining columns' last entries. The single OCR disagreement was escalated, resolved, and has since been independently confirmed. No cell rests on one channel |
