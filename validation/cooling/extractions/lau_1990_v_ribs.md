...
on two independent channels. The V-shaped cases are transcribed and NOT
cleared for implementation -- see "For the reviewer".**
...
| 7 | Table 2 | seven cases, `R`/`G`/`G_bar,Lau` each as `a(e+)^b` -- reproduced below | **two channels on all 42 numbers**, plus a third channel on a cross (full `R` a column, full 7/-V-135 row). One OCR disagreement escalated and resolved. Residual: `G` b rows 5-6 rest on the visual read plus a high-resolution crop, OCR having failed there -- see the review log |
