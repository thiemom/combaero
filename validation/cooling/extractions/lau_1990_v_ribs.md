...
| 7 | Table 2 | seven cases, `R`/`G`/`G_bar,Lau` each as `a(e+)^b` -- reproduced below | **two channels on all 42 numbers, three on the `R` a column, the `G` b column and the 7/-V-135 row.** The single OCR disagreement was escalated, resolved, and has since been independently confirmed. No cell rests on one channel |
