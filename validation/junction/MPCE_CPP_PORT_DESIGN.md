...
| 9 | K is scored at the TARGET q while the solve sits at the achieved q (**9a done, 9b open**) | drift: `three_pb` median 0.134; `mfb_two_pb` 12.6% of rows > 0.25 | **9a done:** `q_converged` populated, carried on the record, aggregated as `median_q_drift`, printed as `dq`; adapters invert their own q transform. **9b open:** score against the measured curve interpolated at the achieved q, which needs a rule for achieved q outside a curve's digitised range | 9b small |
