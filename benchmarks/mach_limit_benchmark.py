...
def build_network(regime: str, mach_target: float) -> FlowNetwork:
    ...
    net.add_element(PipeElement("pipe", "inlet", "plenum", length=length, diameter=d_pipe, roughness=1e-5, regime=pipe_regime))
    net.add_element(OrificeElement("orifice", "plenum", "outlet", Cd=0.6, area=_area(d_pipe*0.8), regime=orifice_regime))
...
if __name__ == "__main__":
    ...
    print(f"{'Strategy':<15} | {'Max Mach':<10} | {'Total Time (s)':<15} | {'Avg Time/Point (s)':<20}")
    print("-" * 65)
    print(f"{'Cold Start':<15} | {res_cold[-1]['mach'] if res_cold else 0:<10.2f} | {time_cold:<15.4f} | {time_cold/len(res_cold) if res_cold else 0:<20.4f}")
    print(f"{'Continuation':<15} | {res_cont[-1]['mach'] if res_cont else 0:<10.2f} | {time_cont:<15.4f} | {time_cont/len(res_cont) if res_cont else 0:<20.4f}")
    ...
    with open("benchmarks/mach_limit_report.md", "w") as f:
        ...
        f.write("| :--- | :--- | :--- | :--- |\n")
        f.write(f"| Cold Start | {res_cold[-1]['mach'] if res_cold else 0:.2f} | {time_cold:.4f} | {time_cold/len(res_cold) if res_cold else 0:.4f} |\n")
        f.write(f"| Continuation | {res_cont[-1]['mach'] if res_cont else 0:.2f} | {time_cont:.4f} | {time_cont/len(res_cont) if res_cont else 0:.4f} |\n\n")

        f.write("## Detail: Continuation Data\n\n")
        f.write("| Mach | dt [s] | norm | max_x0_diff | P_plenum |\n")
        f.write("| :--- | :--- | :--- | :--- | :--- |\n")
        for r in res_cont:
            f.write(f"| {r['mach']:.2f} | {r['dt']:.4f} | {r['norm']:.2e} | {r['max_rel_diff_x0']:.2e} | {r['p_plenum']:.1f} |\n")
