...
def build_network(regime: str, mach_target: float, cfg: SweepConfig) -> FlowNetwork:
    ...
    net.add_element(
        PipeElement(
            ...
            surface=ConvectiveSurface(
                area=np.pi * cfg.d_pipe_hot * cfg.l_pipe, model=SmoothModel()
            ),
        )
    )
    net.add_element(
    ...
    net.add_element(
        PipeElement(
            ...
            regime=pipe_regime,
            surface=ConvectiveSurface(
                area=np.pi * cfg.d_pipe_cold * cfg.l_pipe, model=SmoothModel()
            ),
        )
    )
    net.add_element(
...
if __name__ == "__main__":
    ...
    print("\n--- BENCHMARK RESULTS ---")
    print(
        f"{'Strategy':<15} | {'Success Rate':<15} | {'Total Time (s)':<15} | {'Avg Time/Point (s)':<20}"
    )
    print("-" * 75)
    for r in [results_cold, results_cont]:
        print(
            f"{r['strategy']:<15} | {r['success_rate']:<15.2%} | {r['total_time']:<15.4f} | {r['avg_time']:<20.4f}"
        )

    speedup = (
        results_cold["total_time"] / results_cont["total_time"]
        if results_cont["total_time"] > 0
        else 0
    )
