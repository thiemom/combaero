...
def write_nasa9_chemkin(
    ...
    with open(output_path, "w") as f:
        ...
        f.write(f"{temp_list[0]:8.3f} {temp_list[1]:8.3f} {temp_list[2]:8.3f} {temp_list[-1]:8.3f}\n")
        ...
        for species in species_list:
            ...
            f.write(f"{species_name}{date}{formula}{phase}{T_low:10.3f}{T_high:10.3f}{T_common:5.0f}.{molar_mass:13.7f}{H_ref:15.3f}\n")
            ...
            for range_data in ranges:
                ...
                f.write(f"    {T_min:8.3f}   {T_max:8.3f}7 -2.0 -1.0  0.0  1.0  2.0  3.0  4.0  0.0{H_ref_range:12.3f}\n")
                
                # Coefficient lines (3 coefficients per line, 5 values per coefficient field)
                # Line 1: a1, a2, a3
                f.write(f" {format_scientific_d(coeffs[0])}{format_scientific_d(coeffs[1])}{format_scientific_d(coeffs[2])}\n")
                
                # Line 2: a4, a5, a6
                f.write(f" {format_scientific_d(coeffs[3])}{format_scientific_d(coeffs[4])}{format_scientific_d(coeffs[5])}\n")
                
                # Line 3: a7, a8 (H integration), a9 (S integration)
                # Note: Last line has different format - a7, then two blank spaces, then a8 and a9
                f.write(f" {format_scientific_d(coeffs[6])}                 {format_scientific_d(coeffs[7])}{format_scientific_d(coeffs[8])}\n")
