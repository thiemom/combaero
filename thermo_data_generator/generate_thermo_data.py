...
def generate_cpp_header(
    ...
    output: TextIO,
    json_source: str = "thermo_data_generator/merged_species.json",
) -> None:
    """Generate the C++ header file."""

    # Sort species
    species = sorted(species, key=species_sort_key)
    species_arg = ",".join(sp.name for sp in species)

    output.write("#ifndef THERMO_TRANSPORT_DATA_H\n")
    output.write("#define THERMO_TRANSPORT_DATA_H\n")
    output.write(
        f"""
// !! DO NOT EDIT THIS FILE MANUALLY !!
// This file is generated by thermo_data_generator/generate_thermo_data.py
// Regenerate with:
//   python thermo_data_generator/generate_thermo_data.py \\
//       --json {json_source} --prefer-nasa9 \\
//       --species {species_arg} \\
...
"""
    ...
    for sp in species:
        s = sp.structure
        struct_entries.append(f"{{{s.C_atoms}, {s.H_atoms}, {s.O_atoms}, {s.N_atoms}}}")
...
def main() -> int:
    ...
    # Determine json source path (relative to repo root for banner + sidecar)
    json_source = str(args.json) if args.json else str(args.mechanism)

    # Generate header
    print(f"Generating: {args.output}")
    with open(args.output, "w") as f:
        generate_cpp_header(species_data, nasa_format, f, json_source=json_source)

    # Write sidecar so check-thermo-header.sh can reconstruct the exact invocation
    # without any hardcoded species list.
    species_names_out = [sp.name for sp in sorted(species_data, key=species_sort_key)]
    sidecar_path = Path(__file__).parent / ".generator_state.json"
    sidecar = {
        "species": species_names_out,
        "json_source": json_source,
        "nasa_format": nasa_format.name,
    }
